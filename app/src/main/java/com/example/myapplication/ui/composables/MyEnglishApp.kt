    ...
    Surface(modifier) {
        when (screenState) {
            ...
            "GroupsScreen" -> {
                ...
                LaunchedEffect(Unit) {
                    groups.clear()
                    groups.addAll(queryOverDict(context,"SELECT name FROM `groups`" ))
                }
            ...
            "LessonScreen" -> {
                BackHandler { screenState = "ModeSelectScreen" }
...
fun queryOverDict(context: Context, query: String, selectionArgs: String? = null): List<String> {
